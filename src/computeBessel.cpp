...
bool
computeBessel(std::vector<EMatrix>  &J,const REAL_TYPE& k, const REAL_TYPE& r, 
	      const size_t& lmax, std::ofstream& outStream)
{
  ...
  for(l=1;l<=lmax;l++) {
    ...
    try {
      j_l    = boost::math::sph_bessel(l,rk);
      j_l_m1 = boost::math::sph_bessel(l-1,rk);
      j_l_p1 = boost::math::sph_bessel(l+1,rk);
    }
    catch(...) {
